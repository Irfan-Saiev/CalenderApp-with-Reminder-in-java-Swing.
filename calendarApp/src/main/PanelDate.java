...
public class PanelDate extends javax.swing.JLayeredPane {
    ...
    // Constructor for PanelDate
    public PanelDate(int month, int year) {
        initComponents(); // Initialize components
        this.month = month; // Set the month
        this.year = year; // Set the year
        init(); // Call the init method to initialize the panel
    }
    
    // Initialize the panel
    private void init() {
        // Set the days of the week as title
        mon.asTitle();
        tue.asTitle();
        wed.asTitle();
        thu.asTitle();
        ...
        sat.asTitle();
        sun.asTitle();
        setDate(); // Set the date on the panel
    }

    // Set the date on the panel
    private void setDate() {
        Calendar calendar = Calendar.getInstance(); // Get current calendar instance
        calendar.set(Calendar.YEAR, year); // Set the year
        calendar.set(Calendar.MONTH, month - 1);  // Set the month (January is 0, so subtract 1)
        calendar.set(Calendar.DATE, 1); // Set the date to the first day of the month
        int startDay = calendar.get(Calendar.DAY_OF_WEEK) - 1;  // Get the day of the week for the first day of the month (0-indexed)
        calendar.add(Calendar.DATE, -startDay); // Move the calendar back to the start of the week
        ToDay toDay = getToDay(); // Get the current day
        for (Component com : getComponents()) {
            Cell cell = (Cell) com; // Get each cell component
            if (!cell.isTitle()) { // If it's not a title cell
                cell.setText(calendar.get(Calendar.DATE) + ""); // Set the text of the cell to the current date
                cell.setDate(calendar.getTime()); // Set the date of the cell
                cell.currentMonth(calendar.get(Calendar.MONTH) == month - 1); // Highlight cells that are in the current month
                if (toDay.isToDay(new ToDay(calendar.get(Calendar.DATE), calendar.get(Calendar.MONTH) + 1, calendar.get(Calendar.YEAR)))) {
                    cell.setAsToDay(); // If it's today, mark it as such
                }
    ...
    private ToDay getToDay() {
        ...
        return new ToDay(calendar.get(Calendar.DATE), calendar.get(Calendar.MONTH) + 1, calendar.get(Calendar.YEAR)); // Return a new ToDay object representing today
    }
    ...
    private void initComponents() {
        ...
        fri.setForeground(new java.awt.Color(204, 0, 0));
        fri.setText("Fri");
